...
class blogdetailview(DetailView):
    model = models.Blog
    template_name = 'blog/templates/blog/detail.html'
...
def archives(request, year, month):
   ...
   return render(request, 'blog/templates/blog/index.html', context={'blog_list':blog_list})
...
def search(request):
    ...
    if not q:
        error_msg = "请输入关键词"
        return render(request, 'blog/templates/blog/index.html', {'error_msg': error_msg})
    blog_list = models.Blog.objects.filter(Q(title__icontains=q) | Q(body__icontains=q))
    return render(request, 'blog/templates/blog/index.html', {'error_msg': error_msg, 'blog_list':blog_list})
...
def login(request):
    if request.method == "POST":
        ...
        if user:
            ...
            errormsg="用户名或密码错误！"
            return render(request, 'blog/templates/blog/login.html', {'error':errormsg})
...
def registe(request):
    if request.method == "POST":
        ...
        if form_obj.is_valid(): # 判断校验是否通过
            ...
            user_obj= models.BlogUser.objects.create_user(**form_obj.cleaned_data, is_staff=1, is_superuser=1)
            #obj.is_staff
...
def myindex(request,userid):
   ...
   tabname='mytab'
   return render(request, 'blog/templates/blog/index.html', context={'blog_list':blog_list, 'tabname':tabname})
...
class myindex(ListView):
    ...
    context_object_name = 'blog_list'
    def get_queryset(self):
        loguser=get_object_or_404(models.BlogUser, pk=self.kwargs.get('loguserid'))
...
class authorindex(ListView):
    ...
    context_object_name = 'blog_list'
    def get_queryset(self):
        user=get_object_or_404(models.BlogUser, pk=self.kwargs.get('id'))
